use super::{BufferRef, Completion, File, MemoryIO, OpenFlags, IO};
...
impl File for VfsFileImpl {
    ...
    fn pread(&self, pos: usize, c: Completion) -> Result<()> {
        ...
        let result = {
            let mut buf = r.buf_ptr();
            let count = r.len();
            let vfs = unsafe { &*self.vfs };
            unsafe { (vfs.read)(self.file, buf, count, pos as i64) }
        };
        if result < 0 {
    ...
    fn pwrite(&self, pos: usize, buffer: BufferRef, c: Completion) -> Result<()> {
        ...
        let vfs = unsafe { &*self.vfs };
        let result = unsafe { (vfs.write)(self.file, buffer.as_mut_ptr(), count, pos as i64) };
